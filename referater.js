let referater = [

// tilføj referat nedenfor (husk komma):
{ 
  title: 'Beslutningsreferat',
  id: 'beslutning-dec-24',
  tid: 'December 2024',
  attendees: 'NMA, ITG, BB',
  text: `Læs det officielle beslutningsreferat her: <a href="https://www.sab-bolig.dk/boligafdelinger/alfabetisk-oversigt/askholm/boks2/afdelingsmoeder/09-12-2024-ekstraordinaert">Link til referat</a>
  `
}, { 
  title: 'Bestyrelsesmøde + Markvandring',
  id: 'best-nov-24',
  tid: 'November 2024',
  attendees: 'NMA, ITG, BB',
  text: `<h3>30-årig vedligeholdelsesplan (PPV)</h3>
    <p>
        Maling af opgange er udskudt 5 år, som aftalt, med afsat beløb til løbende reparationer.<br>
        Der er afsat midler til tjek af vandinstallationer i alle lejemål og årlige reparationer.
    </p>

    <h3>Udskiftning af murerfuger</h3>
    <p>
        Foreløbigt udskudt 5 år, med mulighed for yderligere udsættelse. Justeringer foretages, når der er en standard i KAB.
    </p>

    <h3>Økonomiske justeringer</h3>
    <p>
        Henlæggelser er planlagt til at blive hævet med 2% af huslejen, som vedtaget af organisationen - der undersøges hvorvidt dette er nødvendigt.<br>
        Med denne justering viser beregninger, at der først vil være underskud i 2048/2049.
    </p>

    <h3>Budget og møder</h3>
    <p>
        Kundeøkonomen fremsender et budgetudkast i god tid inden afdelingsmødet.<br>
        Der aftales et budgetmøde for at gennemgå den økonomiske situation og diskutere mulige løsninger.
    </p>

    <h3>Aktionsliste</h3>
    <p>
        En opdateret aktionsliste med deadlines for de aftalte punkter fra markvandringen sendes inden jul.
    </p>
  `
}, { 
  title: 'Bestyrelsesmøde',
  id: 'best-sep-24',
  tid: 'September 2024',
  attendees: 'NMA, ITG, BB',
  text: ` <h3>Cykelparkering</h3>
    <p>
        Der opfordres til at gennemføre en cykeloprydning i grundejerforeningen (GF).<br>
        Midler fra GF kan anvendes til opsætning af nye cykelstativer eller ændring af layout og udformning af de eksisterende.<br>
        Bestyrelsen modtager gerne forslag til placering eller ændringer.<br><br>
        Der udsendes et brev med henstilling til ikke at parkere cykler langs facaden, og der modtages gerne forslag til, hvordan cykelparkeringsforholdene kan forbedres.<br><br>
        Cykeloprydning i både kælderen og gården koordineres i regi af gårdlauget.
    </p>

    <h3>Opfølgning på beboerforespørgsler</h3>
    <p>
        Brevet om cykelparkering er behandlet som beskrevet ovenfor.<br><br>
        Problematikken omkring betalingsparkering ved brug af ladestandere i grundejerforeningen tages op på næste bestyrelsesmøde.
    </p>

    <h3>Markvandring</h3>
    <p>
        Planlagte datoer: 21. eller 22. august kl. 17 og 18. september kl. 19. Yderligere markvandring kan ske i november.<br><br>
        Følgende punkter vil blive gennemgået:<br>
        - Vandskader og rustne rør<br>
        - Mulighed for etablering af el-ladere<br>
        - Udluftning i opgange<br>
        - Tjek af lokaler med en checkliste
    </p>

    <h3>Nyt fra GF</h3>
    <p>
        Der er planlagt et møde omkring den grønne plads.<br>
        Forbedring af cykelparkering omkring afdelingen diskuteres, eventuelt i samarbejde med gårdlauget.
    </p>
  `
}, { 
  title: 'Bestyrelsesmøde',
  id: 'best-maj-24',
  tid: 'Maj 2024',
  attendees: 'NMA, ITG, BB',
  text: `<h3>Brev fra bestyrelsen: Indkomne forslag:</h3>
  <p>
    Orangeri i gården - Tages med videre til gårdlaug for mulighedsafklaring
    <br/>
    Gæsteparkering DGV eller Kælder

      <li>Ift. gadeparkering - Tages med til generalforsamling</li>
      <li>Ift. kælderparkeing - Afklaring med KAB/Drift</li>

      Problemer med vinduer:
    <li>Vinduer - Tjek med drift hvad planen for vinduer er.</li>

    Udluftning i opgange:
    <li>Udluftning - Tjek med drift hvad muligheder er? Evt. elektronisk styring eller sæsonbetonet styring (lukket i vinter / åbent om sommeren).</li>
    </p>
    <h3>Generalforsamling GF Sluseholmen </h3>
    <p>Alle forslag til generalforsamlingen kan godkendes af bestyrelsen.</p>

  
  <h3>Driften</h3>
  <p>Der følges op på de åbne punkter med driften.</p>
  
  <h3>Fotografering af afdeling</h3>
  <p>Der planlægges en fotografering af afdelingen i samarbejde med SAB - bestyrelsen hører gerne fra beboere der er interesserede i at stille op. 
  <h4>Skriv til os på <a href="mailto:bestyrelsen@sab-askholm.dk">bestyrelsen@sab-askholm.dk</a></h4>
  </p>
  `
}, { 
  title: 'Bestyrelsesmøde',
  id: 'best-apr-24',
  tid: 'April 2024',
  attendees: 'NMA, ITG, BB',
  text: `<h3>1. Opsamling på Afdelingsmøde</h3>
  <p>Bestyrelsen vil udsende et brev angående følgende emner:</p>
  <ul>
      <li>Opfølgning på afdelingsmødet</li>
      <li>Information om bestyrelsens arbejde, herunder økonomi og planlægning</li>
      <li>Forslag til bestyrelsens arbejde det kommende år med fokus på at øge transparensen for at lette arbejdet</li>
  </ul>
  <br>
  <h3>2. Generalforsamling - Grundejerforeningen</h3>
  <p>Der vil blive fulgt op på generalforsamlingen i grundejerforeningen angående mødetid, mødested og deltagelse fra bestyrelsen.</p>
  <h3>3. Beboerlokale</h3>
  <p>Bestyrelsen overvejer at nedsætte et udvalg til at administrere beboerlokalet med fokus på indretning og inventar.</p>
  `
},    
{       title: 'Brev fra bestyrelsen',
        id: 'brev-apr-24',
        tid: 'April 2024',
        attendees: '-',
        text: ` <h4>Brev fra din afdelingsbestyrelse</h4>
        <p>Kære nabo,</p>
        <p>Vi i bestyrelsen vil gerne give et lille indblik i bestyrelsen, bestyrelsens arbejde, hvorfor det er vigtigt at have en bestyrelse i en almen boligforening, og hvordan du som beboer kan hjælpe bestyrelsen.</p>
        <p>Bestyrelsen består i dag af tre personer: Niklas Munk-Andersen (formand), og bestyrelsesmedlemmerne Ivy Tingting Guan og Birgitte Bidstrup. Bestyrelsen mødes en gang om måneden og drøfter stort og småt.</p>
        <p>Vores primære rolle er at varetage beboernes interesser, og gennem dialog med KAB, Driftscenteret, Gårdlauget, Grundejerforeningen m.fl. sørge for at det er rart og godt at bo i vores afdeling.</p>
        <p>Det er også op til bestyrelsen, at holde øje med økonomien, og sørge for at vi ikke ender med unødige udgifter, og dermed huslejestigninger til følge.</p>
        <p>Bestyrelsen drives på frivillig basis. Vi synes det er vigtigt at foreningen har en bestyrelse, da der ellers bliver truffet beslutninger for os, der ikke nødvendigvis er i vores interesse. Eksempelvis indgår bestyrelsen kontinuerligt i drøftelser med alle interessenter vedr. fordelingen af større udgiftsposter (fx kloakvandskaden i p-kælder i året 2021 samt den pågående nedsivning af regnvand i p-kælder.)</p>
        <p>Et andet eksempel er vedligeholdelse af vores dejlige beboerlokale og gæstebolig, herunder indkøb af nye dyner, tømning af støvsugerpose eller rengøring/afkalkning af opvaskemaskine mv. som bestyrelsen har varetaget indtil videre, men som vi overvejer at betale os fra via servicecenteret, da vi i bestyrelsen ikke har tid til det.</p>
        <h4><strong>Hvordan kan du som beboer hjælpe bestyrelsen ?</strong></h4>
        <ol>
            <li>
                <p><strong>Feedback</strong></p>
                <p>Vi vil meget gerne høre fra dig! Hvordan går det i afdelingen? Hvad fungerer og hvad gør ikke? Er der ting vi skal tage op med servicecenteret? Mangler vi noget i afdelingen?</p>
                <p>Eksempelvis: Fælles arrangementer, faciliteter, rengøring, ladestandere i p-kælderen eller andet.</p>
            </li>
            <li>
                <p><strong>Hjælp med beboerlokale &amp; gæstebolig</strong></p>
                <p>Bestyrelsen er- som dig – generelt ophængt i hverdagen. Derfor har vi brug for hjælp.</p>
                <p>Vi behøver ca. 1-2 personer én gang i kvartalet i ca. 1 time, til at gennemgå lokalerne.</p>
                <p>Vi tager et møde med interesserede. Tilmelding er på ingen måde bindende, hvorfor man altid kan trække sig.</p>
            </li>
        </ol>
        <h4>Skriv til os på <a href="mailto:bestyrelsen@sab-askholm.dk">bestyrelsen@sab-askholm.dk</a></h4>
        <p>Med venlig hilsen</p>
        <p>Din bestyrelse.</p>
        `
    },
    { 
      title: 'Afdelingsmøde',      
      id: 'afdeling-mar-24',
      tid: '13. Marts 2024',
      attendees: 'NMA, ITG, BB, MER',
      text: ` <a href="https://www.sab-bolig.dk/boligafdelinger/alfabetisk-oversigt/askholm/boks2/afdelingsmoeder/13-03-2024">Læs referat her</a> `          
    },
    {       
      title: 'Bestyrelsesmøde',  
      id: 'best-jan-24',
      tid: '3. Januar 2024',
      attendees: 'NMA, JR, BB, MER, ITG',
      text: `
    <h4>Opfølgning på sidste møde:</h4>
        <p>Bestyrelsen har fulgt op på aktiviteter siden sidste møde. 
          Der har været bestyrelseskursus hvor Niklas har deltaget, samt område møde for SAB gruppen Vanløse. Her deltog formanden, Niklas. <br>
          Generelt blev der drøftet forskellige aspekter af SAB, og luft bekymringer for serviceniveauet efter at driften er slået sammen mellem Vanløse afdelingerne og Sluseholmen. 
          <br>
          Driften melder, at de ser på pærer i opgange, samt at der henstilles til at man ikke opbevarer ting i opgangen. Efterladte genstande i opgangen vil blive fjernet af driftspersonalet. 
          </p>
    
        <h4>Vandindtrængen Parkeringskælder: </h4>
        <p>Jens kontakter KAB/SAB ang. hjælp til at søge støtte hos byggeskadefonden/landsbyggefonden
          <br>Gårdlauget indrages for at klarlægge plan for udbedringer. 
          </p>
    
        
        <h4>Cykelparkering Dexter Gordons Vej:</h4>
        <p>Der opleves udfordringer med parkerede ladcykler på Dexter Gordons Vej. Bestyrelsen vil gerne understrege, at parkering langs murene er forbudt, og der henvises til særskilte cykelparkering med stativer, samt parkering af ladcykler i kælderen.
          <br>Der opstartes en dialog med gårdlauget angående cykeloprydning i kælderen for at skabe plads til flere cykler. 
          
        <h4>Kommunikation om bestyrelsen:</h4>
          <p>Bestyrelsen vil arbejde på at udsende information til beboerne ang. bestyrelsesarbejde, samt ophængning af bestyrelsesreferat.</p>

  
        <h4>Næste møde:</h4>
        <p>Næste møde afholdes 7. februar kl. 19, og bestyrelsen modtager som altid gerne forslag el. andet der skal drøftes. 
        </p>
    
        <h4>Evt.:</h4>
    
        <p>Grundejerforeningen afholder generalforsamling i maj, og der påtænkes at planlægge forslag stilling således vi får indflydelse på området. 

          Samtidig bemærkes det også at beboere opfordres til at stille forslag til bestyrelsen såfremt man ønsker at tage noget på afdelingsmødet i marts. </p>
    `
}, 
{   
    title: 'Bestyrelsesmøde',    
    id: 'best-nov-23',
    tid: '8. November 2023',
    attendees: 'NMA, MR, ITG, BB',    
    text: `<h4>Opfølgning på sidste møde:</h4>
    <p>Bestyrelsen har drøftet sager og fulgt op på punkter fra det seneste møde.</p>

    <h4>Markvandring + Økonomi:</h4>
    <p>Formanden informerede om markvandringen, hvor der blev drøftet driftsbudgetter og planlagt vedligeholdelse. Nogle fejl i den nuværende planlagte vedligeholdelse blev identificeret og vil blive rettet inden det endelige budgetforslag for 2024/2025.</p>

    <h5>Tjek op på medfinansiering af udgifter til p-kælder:</h5>
    <p>Jens vil evt. kontakte Byggeskadefonden igen for at klarlægge mulighederne for medfinansiering af udgifter til p-kælder.</p>

    <h5>Generelt om Økonomi:</h5>
    <p>Vi har vendt generelle økonomipunkter, og alle bestyrelsesmedlemmer opfordres til at dykke ned i økonomien på mitKAB bestyrelsesportal. Vi kan evt. undersøge ejendomsværdi ift. ny beskatning - det betyder nok ikke det store så længe ejendommen ikke sælges.</p>

    <h4>Hjemmeside + information fra bestyrelsen:</h4>
    <p>Niklas arbejder på at udsende information fra bestyrelsen og fremhæve hjemmesiden som en central platform for kommende kommunikation. Der refereres til et tidligere skriv udarbejdet af Kristina vedrørende et velkomstbrev.</p>

    <h4>Næste møde:</h4>
    <p>Indkaldelse sendes ud til næste møde den 6. december kl. 19.00.</p>

    <h4>Evt.:</h4>

    <h5>Niveau på service center:</h5>
    <p>Opfordring til beboere om at rapportere god eller dårlig service fra ejendomskontoret.</p>

    <h5>Festlokale:</h5>
    <p>Problemer med støjende fester og overskridelse af husordenen diskuteres. Birgitte arbejder på at udarbejde en "husorden" for lokalerne, som skal sendes til lejere. Forsøg på at skaffe et driftsbudget isoleret for beboerlokale+gæstebolig.</p>
`
}, 
{   
    title: 'Bestyrelsesmøde',    
    id: 'best-okt-23',
    tid: '10. Oktober 2023',
    attendees: 'NMA, MR, ITG, BB',    
    text: `
    <h4>Opfølgning på sidste møde:</h4>
        <p>Bestyrelsen har drøftet sager og fulgt op på punkter fra det seneste møde.</p>
    
        <h4>Information om SAB og organisationen generelt ved Lise Buhelt:</h4>
        <p>Besøg af Lise Buhelt fra SAB, hvor hun præsenterede organisationen og muligheder for samarbejde. Der er potentiale for bedre økonomisk overblik og øget synlighed for bestyrelsen.</p>
    
        <h4>Opsøgende arbejde og bestyrelsens synlighed:</h4>
        <p>Diskussion om nødvendigheden af opsøgende arbejde for at informere nye beboere om bestyrelsens rolle og synlighed. Forslag om at arrangere dialogskabende events som gløgg- eller pølseaftener for at involvere beboerne og øge bestyrelsens synlighed.</p>
    
        <h4>Cykeloprydning og -parkering:</h4>
        <p>Overvejelser om etablering af cykelparkering, især foran børnehuset på Sluseholmen. Bestyrelsens anbefaling er at undgå parkering foran børnehuset på grund af trængsel i morgen- og eftermiddagstimer. I stedet anbefales parkering på hjørnerne ved DGV og TJV.</p>
    
        <h4>Beboerlokale og gæsteværelse:</h4>
        <p>Diskussion om indførelse af depositum som garanti fra gårdlauget i forbindelse med brug af beboerlokaler og gæsteværelse. Behov for yderligere økonomisk analyse af beboerlokale og gæsteværelse.</p>
    
        <h4>Booking system og økonomi:</h4>
        <p>Overvejelser om at implementere et booking system gennem SAB for beboerlokale og gæsteværelse. Nødvendighed af økonomisk gennemgang og opdatering af disse faciliteter.</p>
    
        <h4>Genbrugsstation og affaldssortering:</h4>
        <p>Beslutning om at tage kontakt for at undersøge muligheder for forbedringer i forbindelse med genbrugsstationen og affaldssortering.</p>
    
        <h4>Driftkonti og økonomi:</h4>
        <p>Drøftelse af driftskonti hos KAB. Spørgsmål vedrørende driftsøkonomi rettes til Jesper. Anmodning om økonomisk gennemgang i kommende møder.</p>
    
        <h4>Markvandring og istandsættelse:</h4>
        <p>Planlægning af en markvandring for at identificere områder, der kræver istandsættelse, samt udarbejdelse af budget. Gennemgang af forventningerne til, hvad servicecenteret skal levere til beboerne. Forslag om at forberede og diskutere serviceniveau.</p>
    
        <h4>Bestyrelseskursus og seminar:</h4>
        <p>Overvejelser om deltagelse i kurser og arrangement af et bestyrelsesseminar i Vanløse-området. Mulighed for deltagelse i forskellige kurser gennem KAB, herunder "Ny i bestyrelsen"-kursus og økonomikursus.</p>
    
        <h4>Generelt:</h4>
        <p>Beslutning om at vælge repræsentantskab af bestyrelsen frem for personlig repræsentation, da det giver større fleksibilitet og mulighed for afstemninger i løbet af året. Fastlæggelse af næste mødedato den 8. november eller muligvis den første onsdag i hver måned.</p>`
}, 
{   
    title: 'Bestyrelsesmøde',     
    id: 'best-jun-23',
    tid: '23. Juni 2023',
    attendees: 'NMA, JR, MR, ITG, BB',    
    text: `<h4>Beboerlokale og Gæstebolig</h4>
    <p>Der blev drøftet udfordringer vedrørende drift af beboerlokalet og gæsteboligen.</p>
    
    <h4>Cykelparkering på hjørnerne af karréen</h4>
    <p>Spørgsmål om ejerskab af cykelparkeringen og muligheder for oprydning blev diskuteret. Kontakten til gårdlaug og grundejerforening blev nævnt som mulige løsninger.</p>
    
    <h4>Gårdlaug</h4>
    <p>Bestyrelsen igangsætter arbejde for at afklare økonomien ved udbedring af membranen i gården. Der var også et ønske om bedre kommunikation mellem bestyrelsen og gårdlauget.</p>
    
    <h4>Hjemmeside</h4>
    <p>Bestyrelsen ønsker at få klarhed over ejerskabet af hjemmesiden. Alternativt overvejes oprettelse af en ny hjemmeside. Kontakten til KAB blev nævnt for at få dokumentation eller mulighed for oprettelse af hjemmesiden.</p>
    
    <h4>Synlighed og beboerkommunikation</h4>
    <p>Muligheden for at oprette et nyhedsbrev blev drøftet i forbindelse med hjemmesiden. Der var også ønske om bedre informationsudveksling til beboerne, herunder oplysningskampagner ved indflytning.</p>
    
    <h4>Opgange</h4>
    <p>Der blev diskuteret lugtgener og rygning, bl.a. i opgangene. Muligheden for bedre ventilation for at forbedre indeklimaet blev nævnt.</p>
    
    <h4>Bestyrelsen generelt</h4>
    <p>Niklas Munk-Andersen blev konstitueret som formand, og der var en fælles beslutning om at arbejde på at etablere et mere stabilt og struktureret system for bestyrelsesarbejdet. Dette inkluderer oprettelse af en platform med kontakt, fildeling og digital infrastruktur. Der blev også talt om at undersøge muligheder som Google Workspace eller lignende, samt kontakte KAB vedrørende finansiering af disse initiativer.</p>
    
    `
}];

export default referater;